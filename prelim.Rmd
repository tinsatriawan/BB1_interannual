...
#TO FILTER NON GF DATA TO DAYS WHERE ONLY 30% OR MORE IS PRESENT
daily.nGF <- BB_flux %>%
...
pairs(~ log_co2gC + exp_TS.5 + log_wtd + H + log_ustar + ln_SWC,
      data = data_nGF1,
...
# get goodness of fit stats for multiple subsets 
# nbest = 3 means that the three top with one x variable, two x variables, etc. 
#
co2gC_subset <- regsubsets(log_co2gC~., data = data_nGF1, nbest = 3)
...
# check assumption model 1

model_1 <- lm(log_co2gC ~ TS.5+VPD.y+H+wtd + SWC, data = data_nGF1)
sum_1 <- summary(model_1)

data_nGF1$yhat.1 <- fitted(model_1)
data_nGF1$resid.1 <- residuals(model_1)
data_nGF1$stdresid.1 <- residuals(model_1)/sum_1$sigma

# Diagnotic plots
par(mfrow=c(2,2),cex=1.1,mai=c(1.1,1.1,0.5,0.4))
# Fitted line plot. Predicted y versus observed y.
plot(data_nGF1$yhat.1~data_nGF1$log_co2gC,xlab="Volume/ha",
ylab="yhat",main="Predicted vs Actual Volume/ha",
pch=19)
abline(a=0,b=1,col="red")
# Residual plot
plot(data_nGF1$yhat.1, data_nGF1$resid.1,xlab = "yhat",
ylab = "Residual",main = "Residual Plot",pch = 19)
abline(h=0, col="red")
# Normality plot
qqnorm(data_nGF1$stdresid.1, ylab = "Std. Residual", pch =19)
qqline(data_nGF1$stdresid.1, col ="red")
# Histogram of residuals
hist(data_nGF1$resid.1,breaks = 5,xlab = "Std. Residual",
...
data_nGF2 <- data_nGF %>%
  mutate(co2gC = exp(co2gC)^0.2,
         exp_TS.5 = (exp(TS.5)^0.2), 
         log_wtd = log10(wtd + 1 - min(wtd)), 
         ln_SWC = log(SWC)) %>% 
  dplyr::select(-c(ch4gC, date, SWC, wtd, TS.5)) 
...
model_2 <- lm(co2gC ~ exp_TS.5+log_wtd+ln_SWC+H+ustar, data = data_nGF2)
...
# Histogram of residuals
hist(data_nGF1$resid.2,breaks = 5,xlab = "Std. Residual",
main = "Std. Residuals", col="grey", border="black")
par(mfrow=c(1,1),cex=1.1,mai=c(1.1,1.1,1.0,1.0))
