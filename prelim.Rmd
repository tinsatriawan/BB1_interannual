...
ET <- BB_met%>%
ggplot() +
  geom_bar(data = met_monthly_anom, aes(x = floor_date, y = ET*2), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = rollmean(ET, 7, fill = NA))) +
  scale_y_continuous(
    # limits = c(0, 30),
    name = bquote("E (mm)"),
    sec.axis = sec_axis(~./2, name = bquote(Delta*~"E (mm)"))
    ) +
  labs(x = "") +
  scale_x_date(date_breaks = "1 year", 
               date_labels = "%b %Y") +
...
x_break <- met_year$order[!is.na(met_year$day_label)] [1:72]  # 30 * 7  
...
# extract phenology dates
pheno_Reco <- calc_phenology(INPUT_Reco, use.rough = T)
...
met_loop <- c("TS.5", "Ta", "VPD", "wtd", "PARin", "Precip", "GPPgC_f_RF", "min_TS.5", "max_TS.5", "min_Ta", "max_Ta", "min_wtd", "max_wtd", "PA", "RH", "day_wtd", "day_wtd2", "day_precip")
