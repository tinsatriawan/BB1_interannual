...
month.abb.level <- c("Oct","Nov", "Dec", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep")
...
monthly_sd <- monthly %>% 
  ddply("month_local", summarize, 
       NEEgC_f = sd(NEEgC_f, na.rm = T),
       NEEgC_f_RF = sd(NEEgC_f_RF, na.rm = T), 
       GPPgC_f = sd(GPPgC_f, na.rm = T),
       GPPgC_f_RF = sd(GPPgC_f_RF, na.rm = T), 
       RecogC = sd(RecogC, na.rm = T),
       CH4gC_f = sd(CH4gC_f, na.rm = T),
       CH4gC_f_RF = sd(CH4gC_f_RF, na.rm = T))

#### calculate monthly anomalies #####
monthly_anom <- monthly
for(i in 1:nrow(monthly)){ 
  monthly_anom[i, c(3:9)] <- monthly_anom[i, c(3:9)] -  
    monthly_means[which(monthly_means$month_local == monthly_anom$month_local[i]), c(2:8)]
}
...
```{r plotting non gapfilled daily fluxes, message = F, warning = F}
...
```{r plotting gapfilled daily fluxes, message = F, warning = F}
...
met_monthly <- BB_met %>% 
  ddply("floor_date", summarize, 
      ...
      SWin = mean(SWin, na.rm = T),  # W m-2 day-1
      ...
      ET = sum(ET, na.rm = T),  # mm/month
      wtd = mean(wtd, na.rm = T), 
      br = mean(br, na.rm = T),  # unitless
      albedo = mean(albedo, na.rm = T), # unitless
      LST = mean(LST, na.rm = T),  # land surface temp 
      G = mean(G, na.rm = T),
      G_corr = mean(G_corr, na.rm = T), 
      H = mean(H, na.rm = T),  # W m-2
      LE = mean(LE, na.rm = T), # W m-2
      LWin = mean(LWin, na.rm = T), # W m-2 
      LWout = mean(LWout, na.rm = T) ,  # W m-2 
      SWout = mean(SWout, na.rm = T),   # W m-2
      Rn = mean(Rn, na.rm = T),  # W m-2
      PAR_SWin = mean(PAR_SWin, na.rm = T)  #no unit
      ) %>%
  mutate(month_local = month(floor_date))

# calculate monthly means (averaged across all years) met variables
...
# calculate stdev (across all years) for monthly met variables
met_monthly_sd <- met_monthly %>% 
  ddply("month_local", summarize, 
      TS.5 = sd(TS.5, na.rm = T),  # soil temp
      TS.10 = sd(TS.10, na.rm = T),
      TS.50 = sd(TS.50, na.rm = T),
      RH = sd(RH, na.rm = T),  # relative humidity
      PARin = sd(PARin, na.rm = T),  
      SWin = sd(SWin, na.rm = T),
      VPD.y = sd(VPD.y, na.rm = T),
      Precip = sd(Precip, na.rm = T),
      PA_2M = sd(PA_2M, na.rm = T),  # barrometric pressure from met
      Ta = sd(Ta, na.rm = T),  # air temp at 2 m
      SWC = sd(SWC, na.rm = T), 
      ET = sd(ET, na.rm = T),
      wtd = sd(wtd, na.rm = T), 
      br = sd(br, na.rm = T),  # unitless
      albedo = sd(albedo, na.rm = T), # unitless
      LST = sd(LST, na.rm = T),  # land surface temp 
      G = sd(G, na.rm = T), 
      G_corr = sd(G_corr, na.rm = T), 
      H = sd(H, na.rm = T),  # W m-2
      LE = sd(LE, na.rm = T), # W m-2
      LWin = sd(LWin, na.rm = T), # W m-2 
      LWout = sd(LWout, na.rm = T) ,  # W m-2 
      SWout = sd(SWout, na.rm = T),   # W m-2
      Rn = sd(Rn, na.rm = T),  # W m-2
      PAR_SWin = mean(PAR_SWin, na.rm = T)  #no unit
      )
...
#   }
start <- 2 
end <- ncol(met_monthly_anom)-1
for(i in 1:nrow(met_monthly_anom)){
  index <- which(met_monthly_means$month_local == met_monthly_anom$month_local[i])
  met_monthly_anom[i, c(start:end)] <- met_monthly_anom[i,  c(start:end)] -
    met_monthly_means[index, c(start:end)]
  # print(i)
  }
...
#### calculate anomalies from climate normals 

climate_normals_anom <- data.frame(floor_date = NA, 
                              month_local = NA,
                              precip_anom = NA, 
                              Ta_anom = NA) 

for(i in 1:nrow(met_monthly)){
  index <- which(climate_normals$month == met_monthly$month_local[i])
  climate_normals_anom[i, "floor_date"] <- as.Date(met_monthly$floor_date[i])
  climate_normals_anom[i, "month_local"] <- met_monthly$month_local[i]
  climate_normals_anom[i, "precip_anom"] <- met_monthly$Precip[i] - climate_normals$precip_avg[index]
  climate_normals_anom[i, "Ta_anom"] <- met_monthly$Ta[i] - climate_normals$T_avg[index]
}

climate_normals_anom <- climate_normals_anom %>% 
  mutate(date = as.Date(floor_date)) %>% 
  set_period() %>% 
  set_season(date = climate_normals_anom$date)

```

```{r plot anomalies from climate normals, fig.height = 8, fig.width = 8}

climate_normals_anom %>% 
  ggplot() + 
  geom_point(aes(x = Ta_anom, y = precip_anom, 
                 colour = factor(period), 
                 shape = factor(month_local)), 
             size = 4) + 
  scale_shape_manual(values=1:nlevels(factor(climate_normals_anom$month_local))) +
  geom_abline(intercept = 0, slope = 0) + 
  geom_vline(xintercept = 0) + 
  scale_x_continuous(limits = c(-5, 5),  name = bquote(T[a]~'anomalies('*degree*'C)')) +
  scale_y_continuous(limits = c(-130, 130), name = "P anomalies (mm) ") +
  theme_bw () + 
  theme(text = element_text(size = 12)) + 
  labs(col = "period", shape = "month")
...
met_monthly_anom %>% 
  ...
  set_period() %>%
  ggplot() + 
  geom_point(aes(x = Ta, y = wtd *100, 
                 colour = factor(period), 
                 shape = factor(month_local)), 
             size = 4) + 
  scale_shape_manual(values=1:nlevels(factor(met_monthly_anom$month_local))) +
  geom_abline(intercept = 0, slope = 0) + 
  geom_vline(xintercept = 0) + 
  scale_x_continuous(limits = c(-3.5, 3.5), name =bquote(T[a]~'anomalies('*degree*'C)')) +
  scale_y_continuous(limits = c(-9, 9), name = "WTD anomalies (cm)") +
  theme_bw() + 
  labs(col = "period", shape = "month")

```

```{r plotting daily met variables, message = F, warning = F}
...
(PA <- plot_daily(data = BB_met, 
                  variable = PA_2M, 
                  label = "Barrometric pressure (kPa)", 
                  ma = 1)) 
...
```{r plot daily fluxes and monthly anomalies, fig.height = 8, fig.width = 8, message = F, warning = F}
...
flux_labels <- c(
  bquote(NEE~'(g C' ~m^-2~d^-1~ ')'),
  bquote(GPP~'(g C' ~m^-2~d^-1~')'),
  bquote(Reco~'(g C' ~m^-2~d^-1~')'),
  bquote(CH[4]~'(g C' ~m^-2~d^-1~')')
)
...
flux_year <- daily %>%
  ...
  ungroup() %>%
  mutate(
          NEEgC_f_RF = rollmean(NEEgC_f_RF, 14, fill = NA),
          GPPgC_f_RF = rollmean(GPPgC_f_RF, 14, fill = NA),
          RecogC = rollmean(RecogC, 14, fill = NA), 
          CH4gC_f_RF = rollmean(CH4gC_f_RF, 14, fill = NA), 

         ) %>%
  select(c(date, period, NEEgC_f_RF, GPPgC_f_RF, RecogC, CH4gC_f_RF)) %>%
  pivot_longer(cols = c(NEEgC_f_RF, GPPgC_f_RF, RecogC, CH4gC_f_RF), names_to = "variable", values_to = "value") %>%
...
x_break <- flux_year$order[!is.na(flux_year$day_label)][1:60]
x_label <- flux_year$day_label[!is.na(flux_year$day_label)][1:60]
x_label
windows()
flux_year %>% 
...
```{r plot monthly flux boxplot, message = F, warning = F}

### NEE ###
jpeg("plots/monthly_NEE.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(NEEgC_f_RF~factor(month.abb[month_local], levels = month.abb.level),
        data=daily,
        xlab="Month",
...
### GPP ###
jpeg("plots/monthly_GPP.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(GPPgC_f_RF~factor(month.abb[month_local], levels = month.abb.level),
        data=daily,
        xlab="Month",
...
### Reco ###
jpeg("plots/monthly_Reco.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(Reco~factor(month.abb[month_local], levels = month.abb.level),
        data=daily,
        xlab="Month",
        ylab=expression('Reco (g C' ~m^-2~ d^-1*')'),
        col="orange",
        border="brown"
)
abline(a=0,b=0,col="black")
...
```{r plot daily met and monthly anomalies, fig.height = 10, fig.width = 8, message = F, warning = F} 
# met_labels <- c(
#   bquote(T[s]~'at 5 cm'),
...
ggplot() +
  ...
  geom_bar(aes(x = date, y = Precip), stat = "identity", fill = "black") +
  scale_y_continuous(
    # limits = c(0, 30),
...
ET <- BB_met%>%
ggplot() +
  geom_bar(data = met_monthly_anom, aes(x = floor_date, y = ET*0.2), fill = "gray", stat = "identity") +
  geom_bar(aes(x = date, y = ET), fill = "black", stat = "identity") +
  scale_y_continuous(
    # limits = c(0, 30),
    name = bquote("ET (mm)"~d^-1~")"),
    sec.axis = sec_axis(~./0.2, name = bquote(Delta*~"ET (mm)"~month^-1~")"))
    ) +
  labs(x = "") +
  scale_x_date(date_breaks = "1 year", 
               date_labels = "%b %Y") +
...
SWin <- BB_met%>%
ggplot() +
  geom_bar(data = met_monthly_anom, aes(x = floor_date, y = SWin*4), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = rollmean(SWin, 7, fill = NA))) +
  scale_y_continuous(
    # limits = c(0, 30),
    name = bquote(SW["in"]~'(W'~m^{-2}~')'),
    sec.axis = sec_axis(~./4, name = bquote(Delta*~SW["in"]~'(W'~m^{-2}~')'))
    ) +
  labs(x = "") +
  scale_x_date(date_breaks = "1 year", 
               date_labels = "%b %Y") +
...
PA_2M <- BB_met%>%
ggplot() +
  # geom_bar(data = met_monthly_anom, aes(x = floor_date, y = PA_2M*100), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = rollmean(PA_2M, 7, fill = NA))) +
  # scale_y_continuous(
  #   # limits = c(0, 30),
  #   name = bquote("Pa (kPa)"),
  #   sec.axis = sec_axis(~./100, name = bquote(Delta*~"Pa (kPa)"))
  #   ) +
  # labs(x = "") +
  scale_x_date(date_breaks = "1 year",
               date_labels = "%b %Y") +
...
energy_fluxes <- BB_met%>%
ggplot() +
  # geom_bar(data = met_monthly_anom, aes(x = floor_date, y = PA_2M*100), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = rollmean(Rn, 7, fill = NA), col = "Rn"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(LE, 7, fill = NA), col = "LE"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(H, 7, fill = NA), col = "H"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(G_corr, 7, fill = NA), col = "G"), size = 0.8) +
  scale_color_manual(values = c("Rn" = "magenta", 
                                "LE" = "#00798c", 
                                "H" = "#d1495b", 
                                "G" = "orange")) +
  labs(y = bquote('Energy fluxes (W'~m^{-2}~')'), 
       col = "") +
  scale_x_date(date_breaks = "1 year",
...
ggplot() +
  # geom_bar(data = met_monthly_anom, aes(x = floor_date, y = PA_2M*100), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = rollmean(LWin, 7, fill = NA), col = "LWin"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(LWout, 7, fill = NA), col = "LWout"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(SWin, 7, fill = NA), col = "SWin"), size = 0.8) +
  geom_line(aes(x = date, y = rollmean(SWout, 7, fill = NA), col = "SWout"), size = 0.8) +
  scale_color_manual(values = c("LWin" = "magenta", 
                                "LWout" = "#00798c", 
                                "SWin" = "#d1495b", 
                                "SWout" = "orange")) +
  labs(y = bquote('Radiation (W'~m^{-2}~')'), 
       col = "") +
  scale_x_date(date_breaks = "1 year",
               date_minor_breaks = "1 month",
               date_labels = "%b %Y") +
...
albedo <- BB_met%>%
ggplot() +
  geom_bar(data = met_monthly_anom, aes(x = floor_date, y = albedo*4), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = albedo)) +
  scale_y_continuous(
    breaks = c(-0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8), 
    name = bquote('Albedo'),
    sec.axis = sec_axis(~./4, name = bquote(Delta*~'Albedo'))
    ) +
  labs(x = "") +
  scale_x_date(date_breaks = "1 year", 
               date_labels = "%b %Y") +
...
PAR_SWin <- BB_met%>%
ggplot() +
  geom_bar(data = met_monthly_anom, aes(x = floor_date, y = PAR_SWin*4), fill = "gray", stat = "identity") +
  geom_line(aes(x = date, y = PAR_SWin)) +
  scale_y_continuous(
    name = bquote('PAR/'~SW['in']),
    sec.axis = sec_axis(~./4, name = bquote(Delta*~'PAR/'~SW['in']))
    ) +
  labs(x = "") +
...
ggsave("plots/ET.png")
print(SWin)
ggsave("plots/SWin.png")
print(energy_fluxes)
ggsave("plots/energy_fluxes.png")
print(albedo)
ggsave("plots/albedo.png")
print(PAR_SWin)
ggsave("plots/PAR_SWin.png")
print(radiation)
ggsave("plots/radiation.png")
# ggplotly(water)

```

```{r plot monthly met boxplot, message = F, warning = F}

### soil temperature ###
jpeg("plots/monthly_TS.5.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(TS.5~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
        ylab="TS.5 (C)",
        col="orange",
...
### relative humidity ###
jpeg("plots/monthly_RH.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot_RH <- boxplot(RH~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
...
### barrometric pressure ###
jpeg("plots/monthly_PA_2M.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(PA_2M~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
        ylab="Barrometric pressure (kPa)",
        col="orange",
        border="brown"
)
dev.off()
...
### SWC ###
jpeg("plots/monthly_SWC.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(SWC~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
...
### SWC ###
jpeg("plots/monthly_Ta.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(Ta~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
...
### PARin ###
jpeg("plots/monthly_PARin.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(PARin~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
        ylab=expression("PAR ("*mu*"mol"~m^{-2}~s^{-2}*")"),
        col="orange",
        border="brown"
)
dev.off()
...
### VPD ###
jpeg("plots/monthly_VPD.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(VPD.y~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
        ylab="VPD",
        col="orange",
...
### WTD ###
jpeg("plots/monthly_WTD.jpg", width = 900, height = 350, units = "px", quality = 120)
boxplot(wtd*100~factor(month.abb[month_local], levels = month.abb.level),
        data=BB_met,
        xlab="Month",
        ylab="WTD(cm)",
        col="orange",
...
met_labels <- c(
  expression('Air temperature ('*~degree*C*')'),
  expression('Soil temperature ('*~degree*C*')'),
  expression("PAR ("*mu*"mol"~m^{-2}~s^{-2}*")"),
  expression("E (mm)"),
  "WTD (m)",
  "Precip (mm)" 
...
met_year <- BB_met %>%
  ...
  ungroup() %>%
  # mutate(
  #        # Ta = rollmean(Ta, 14, fill = NA),
  # # #        PARin = rollmean(PARin, 14, fill = NA),
  # # #        SWin = rollmean(SWin, 14, fill = NA),
  # #        ET = rollmean(ET, 14, fill = NA)
  # # #        Precip = rollmean(Precip, 14, fill = NA)
  #        ) %>%
  select(c(date, period, Ta, TS.5, PARin, ET, wtd, Precip)) %>%
  pivot_longer(cols = c(Ta, TS.5, PARin, ET, wtd, Precip), names_to = "variable", values_to = "value") %>%
  mutate(variable_lab = factor(variable,
                               levels = c("Ta", "TS.5", "PARin", 'ET', 'wtd', 'Precip'),
                               labels = met_labels)) %>%
  group_by(variable,period) %>%
  arrange(date) %>%
  mutate(order = row_number()) %>%
...
x_break <- met_year$order[!is.na(met_year$day_label)] [1:72]  # 6 (number of variable) * 12 (month)
x_label <- met_year$day_label[!is.na(met_year$day_label)]
x_label
...
met_year %>% 
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside")
ggsave("plots/met_yearly.png")



## using daily data for flux var (LE, H)

fluxmet_labels <- c(
  expression('LE ('~W~m^2~')'),
  expression('H ('~W~m^2~')'),
  expression("ET (mm "~d^{-1}~")"), 
  expression('G ('~W~m^2~')')
)
...
fluxmet_year <- BB_met %>%
  ...
  ungroup() %>%
  mutate(
         LE = rollmean(LE, 14, fill = NA),
         H = rollmean(H, 14, fill = NA),
         ET = rollmean(ET, 14, fill = NA),
         G_corr = rollmean(G_corr, 14, fill = NA),
         ) %>%
  select(c(date, period, LE, H, ET, G_corr)) %>%
  pivot_longer(cols = c(LE, H, ET, G_corr), names_to = "variable", values_to = "value") %>%
  mutate(variable_lab = factor(variable,
                               levels = c("LE", "H", "ET", 'G_corr'),
                               labels = fluxmet_labels)) %>%
  group_by(variable,period) %>%
  arrange(date) %>%
  mutate(order = row_number()) %>%
  ungroup() %>%
...
x_break <- fluxmet_year$order[!is.na(fluxmet_year$day_label)][1:60]
x_label <- fluxmet_year$day_label[!is.na(fluxmet_year$day_label)][1:60]
x_label
windows()
fluxmet_year %>% 
  ggplot(aes(x = order, y = value, color = factor(period), group = factor(period))) + 
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside")
ggsave("plots/fluxmet_yearly.png")  
...
met2_year <- BB_met %>%
  ...
  mutate(
         LE = rollmean(LE, 14, fill = NA),
         TS.5 = rollmean(TS.5, 14, fill = NA),
         wtd = rollmean(wtd, 14, fill = NA),
         PA_2M = rollmean(PA_2M, 14, fill = NA),
         RH = rollmean(RH, 14, fill = NA),
         ) %>%
  select(c(date, period, LE, TS.5, wtd, PA_2M, RH)) %>%
  pivot_longer(cols = c(LE, TS.5, wtd, PA_2M, RH), names_to = "variable", values_to = "value") %>%
  mutate(variable_lab = factor(variable,
                               levels = c("LE", "TS.5", "wtd", "PA_2M", "RH"),
                               labels = met2_label)) %>%
  group_by(variable,period) %>%
  arrange(date) %>%
  mutate(order = row_number()) %>%
...
x_break <- met2_year$order[!is.na(met2_year$day_label)][1:60]
x_label <- met2_year$day_label[!is.na(met2_year$day_label)][1:60]
x_label
windows()
met2_year %>% 
  ggplot(aes(x = order, y = value, color = factor(period), group = factor(period))) + 
...
met_an_means <- BB_met %>%
  ...
  summarise(min_TS.5 = min(TS.5, na.rm = T), 
      max_TS.5 = max(TS.5, na.rm = T),
      min_Ta = min(Ta, na.rm = T), 
      max_Ta = max(Ta, na.rm = T),
      min_wtd = min(wtd, na.rm = T), 
      max_wtd = max(wtd, na.rm = T), 
      day_wtd = count(wtd > 0, na.rm = T), 
      day_wtd2 = count(wtd < -0.1, na.rm = T),
      wtmd = sum(wtd < 0, na.rm = T), 
      wtms = sum(wtd > 0, na.rm = T),
...
write.csv(flux_an_sum, "df/flux_annual_summary.csv")
write.csv(flux_season_sum, "df/flux_season_summary.csv")
write.csv(met_an_means, "df/met_annual_summary.csv")
write.csv(met_season_sum, "df/met_season_summary.csv")
write.csv(met_monthly_means, "df/met_monthly_means.csv")
write.csv(monthly_means, "df/flux_monthly_means.csv")
write.csv(met_monthly_sd, "df/met_monthly_sd.csv")
write.csv(monthly_sd, "df/flux_monthly_sd.csv")
...
INPUT_GPP <- check_input(t = as.Date(weekly.nGF$floor_date),
                     y = (weekly.nGF$GPP_f_RF))  # use this
                     # y = weekly.nGF$co2gC + weekly.GF$Reco)
                     

NEE_use <- weekly.GF
NEE_use[is.na(weekly.nGF$co2gC), -1] <- NA
INPUT_NEE <- check_input(t = as.Date(weekly.nGF$floor_date),
                     # y = -(weekly.nGF$co2gC) + 1)
                     y = -(NEE_use$NEE_f_RF) + 1)
                     # y = -(weekly.GF$NEE_f_RF) + 1)
...
pheno_GPP <- calc_phenology(INPUT_GPP, use.rough = T)
...
get_pheno(pheno_GPP$fit,  method = "AG", wFUN = wBisquare, IsPlot = T)
...
# plot transition dates 
l_pheno <- get_pheno(pheno_NEE$fit, method = "AG", wFUN = wBisquare, IsPlot = T)
...
pheno_index <- data.frame(period = pheno_GPP$pheno$flag,
                          ...
                          Reco_start = pheno_Reco$pheno$TRS1.sos, 
                          Reco_end = pheno_Reco$pheno$TRS1.eos,
                          NEE_peak = pheno_NEE$pheno$DER.pos, 
                          GPP_peak = pheno_GPP$pheno$DER.pos, 
                          Reco_peak = pheno_Reco$pheno$DER.pos) %>%
  ...
  filter(method == "AG") %>%
  mutate(period = c("2016-2017", "2017-2018", "2018-2019", "2019-2020", "2020-2021"))  # remove 2015


NEE_fit_forplot <- NEE_fit_forplot %>% filter(meth == "AG")
GPP_fit_forplot <- GPP_fit_forplot %>% filter(meth == "AG")

plot_ly(data = weekly.nGF, x = ~floor_date, y = ~-co2gC, type = "scatter", mode = "markers", name = "NEE") %>% 
  add_trace(data = weekly.nGF, x = ~floor_date, y = ~GPP_f_RF, type = "scatter", mode = "markers", name = "GPP") %>% 
  add_trace(data = weekly.nGF, x = ~floor_date, y = ~Reco, type = "scatter", mode = "markers", name = "Reco") %>% 
  ...
  add_trace(data = weekly.GF, x = ~floor_date, y = ~-NEE_f_RF, type = "scatter", mode = "markers", name = "NEE.gf" )

  

write.csv(pheno_index, "df/pheno_index.csv")
...
# need to add
corr_NGS_pheno <- left_join(pheno_index, filter(flux_season_sum, season == "non-growing season"))%>%
  mutate(year = substr(period, 1, 4))
corr_GS_pheno <- left_join(pheno_index, filter(flux_season_sum, season == "growing season"))%>%
  mutate(year = substr(period, 1, 4))


flux_loop <- c("NEEgC_f_RF", "RecogC", "GPPgC_f_RF", "CH4_gC_RF")
met_loop <- c("TS.5", "Ta", "VPD", "wtd", "PARin", "Precip", "GPPgC_f_RF", "min_TS.5", "max_TS.5", "min_Ta", "max_Ta", "min_wtd", "max_wtd", "PA", "RH", "day_wtd", "day_wtd2", "day_precip", "wtmd", "wtms", "ET")
phen_loop <- c("CUP_start",
               "CUP", 
               # "CUP_DER", "CUP_TRS", "CUP_G",
               "GSP_DER", "GSP_TRS", "GSP_G", "senescence_maturity",
               "RGR", "summer_lag", "autumn_lag", "NEE_peak", "GPP_peak", "Reco_peak")
...
for (i in flux_loop) {
  for (j in phen_loop) { 
    eval(parse(text = paste0(i, '_vs_' , j, '<- corr_GS_pheno %>% ggplot(aes_string(x = "', j,'", y = "',i,'")) + 
    geom_point(size = 3) + 
  geom_smooth(method = "lm", se = F, col = "black") +
  ggtitle("GS flux vs annual PI") + 
  geom_text(aes(label=year), size = 3, hjust=0.5, vjust=-0.8) +
  labs(x = "', j,'", y = "',i,'",  col= "") +  
  stat_cor(size = 3.5) +
  theme_bw()+ 
  theme(legend.position="none")')))
    eval(parse(text = paste0("print(",i,"_vs_",j,")")))
    eval(parse(text = paste0("ggsave('plots/corr_phen_vs_GS",i,"_vs_",j,".png')")))
  }
}
...
for (i in flux_loop) {
  for (j in phen_loop) { 
    eval(parse(text = paste0(i, '_vs_' , j, '<- corr_NGS_pheno %>% ggplot(aes_string(x = "', j,'", y = "',i,'")) + 
    geom_point(size = 3) + 
  geom_smooth(method = "lm", se = F, col = "black") +
  ggtitle("NGS flux vs annual PI") + 
  geom_text(aes(label=year), size = 3, hjust=0.5, vjust=-0.8) +
  labs(x = "', j,'", y = "',i,'",  col= "") +  
  stat_cor(size = 3.5) +
  theme_bw()+ 
  theme(legend.position="none")')))
    eval(parse(text = paste0("print(",i,"_vs_",j,")")))
    eval(parse(text = paste0("ggsave('plots/corr_phen_vs_NGS",i,"_vs_",j,".png')")))
  }
}
...
RS_indices_daily <- RS_indices_daily[-15, ] # delete a row (data for April 13 2016, cloudy)
...
# calculate annual RS indices (mean, peak, median) for growing season

RS_GS <- RS_indices %>% 
  mutate(month_local = month(date)) %>% 
  filter(month_local  %in% c(4, 5, 6, 7, 8, 9)) %>%
  set_period() %>% 
  drop_na()
...
RS_GS_summary <- RS_GS %>% 
  ddply("period", summarise,
    ...
    NDVI_median = median(NDVI, na.rm = T), 
    NDVI_max = max(NDVI, na.rm = T), 
    NDVI_min = min(NDVI, na.rm = T),
    GPP_mean = mean(GPP_7, na.rm = T), 
    GPP_median = median(GPP_7, na.rm = T), 
    GPP_max = max(GPP_7, na.rm = T), 
    GPP_min = min(GPP_7, na.rm = T),
    LST_mean = mean(LST, na.rm = T), 
    LST_median = median(LST, na.rm = T), 
    LST_max = max(LST, na.rm = T), 
    LST_min = min(LST, na.rm = T))


RS_GS %>% 
  ggplot(aes(x = NDVI, y = GPP_7)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = F) +
  theme_bw()

summary(lm(GPP_7~NDVI, RS_GS))
summary(lm(GPP_7~NDVI*period, RS_GS))
anova(lm(GPP_7~NDVI, RS_GS), lm(GPP_7~NDVI*period, RS_GS))
# relationship between GPP and NDVI didn't change from year to year


plot(RS_GS_summary$NDVI_mean, RS_GS_summary$GPP_mean)
plot(RS_GS_summary$NDVI_median, RS_GS_summary$GPP_median)
plot(RS_GS_summary$NDVI_max, RS_GS_summary$GPP_max)
plot(RS_GS_summary$NDVI_min, RS_GS_summary$GPP_min)
