...
dir_rf <- "G:/.shortcut-targets-by-id/1txCh9lZ7VGCujXGvBaJCMVnuxT-65q4K/Micromet Lab/Projects/2014-Burns\ Bog/Flux-tower (1)/flux_data/for_uncertainty/RF_results"

dir_unc <- "G:/.shortcut-targets-by-id/1txCh9lZ7VGCujXGvBaJCMVnuxT-65q4K/Micromet Lab/Projects/2014-Burns\ Bog/Flux-tower (1)/flux_data/for_uncertainty"
...
GPP_MDS2_cumsum <- sdAggDayUstar_GPP %>% 
  filter(date >= "2016-02-21 00:30:00" & date <"2016-06-20 00:30:00") %>%
  calculate_cumsum_unc(data = .,
                             ind = ustar_ind, 
                             variable = "GPP_f", 
...
GPP_MDS3_cumsum <- sdAggDayUstar_GPP %>% 
  filter(date >= "2016-06-20 00:30:00" & date < "2016-10-22 00:30:00") %>% 
  calculate_cumsum_unc(data = .,
                             ind = ustar_ind, 
                             variable = "GPP_f", 
                             start_mean = 0, 
...
GPP_MDS4_cumsum <- sdAggDayUstar_GPP %>% 
  filter(date >= "2017-01-21 00:30:00" & date < "2017-06-20 00:30:00") %>% 
  calculate_cumsum_unc(data = .,
                       ind = ustar_ind, 
                       variable = "GPP_f", 
...
period <- unique(sdAggDay_RF_FCH4$period)[1:6]
...
write.csv(annual_budget, paste0(here::here(), "/annual_budget.csv"))


```

```{r uncertainty analysis (original), eval = F}

# ###### wrangling REddyProcOutput ######
# 
# REddyProcOutput <- REddyProcOutput %>% 
#   mutate(date = as.Date(as.POSIXct(sDateTime - 15*60))) %>% # midnight belongs to the previou
#   mutate(year_local = year(date), 
#          month_local = month(date), 
#          floor_date = floor_date(date, "month")
#   ) %>% 
...
# 
# # set year period
# y_2015_2016 <- REddyProcOutput$date >= "2015-06-20" & REddyProcOutput$date < "2016-06-20"
# y_2016_2017 <- REddyProcOutput$date >= "2016-06-20" & REddyProcOutput$date < "2017-06-20"
# y_2017_2018 <- REddyProcOutput$date >= "2017-06-20" & REddyProcOutput$date < "2018-06-20"
# y_2018_2019 <- REddyProcOutput$date >= "2018-06-20" & REddyProcOutput$date < "2019-06-20"
# y_2019_2020 <- REddyProcOutput$date >= "2019-06-20" & REddyProcOutput$date < "2020-06-20"
# y_2020_2021 <- REddyProcOutput$date >= "2020-06-20" & REddyProcOutput$date < "2021-06-20"
# 
# REddyProcOutput$period[y_2015_2016] <- "2015-2016"
# REddyProcOutput$period[y_2016_2017] <- "2016-2017"
# REddyProcOutput$period[y_2017_2018] <- "2017-2018"
# REddyProcOutput$period[y_2018_2019] <- "2018-2019"
# REddyProcOutput$period[y_2019_2020] <- "2019-2020"
# REddyProcOutput$period[y_2020_2021] <- "2020-2021"
# 
# # set season 
# GS <- REddyProcOutput$month_local %in% c(4, 5, 6, 7, 8, 9)
# NGS <- REddyProcOutput$month_local %in% c(10, 11, 12, 1, 2, 3)
...
# 
# ## summary
# summary(REddyProcOutput$NEE_uStar_fsd)
# plot(NEE_uStar_fsd ~ NEE_uStar_fall, slice(REddyProcOutput, sample.int(nrow(REddyProcOutput),400)))
# 
# 
# ###### Random Uncertainty ##########
# 
...
#         nEff = computeEffectiveNumObs(resid, effAcf = !!autoCorr, na.rm = TRUE),
#         NEE = mean(NEE_uStar_f, na.rm = TRUE)* d.avg * co2_conv,
#         sdNEE = if (nEff <= 1)
#           NA_real_
#           else sqrt(mean(NEE_uStar_fsd^2, na.rm = TRUE) / (nEff - 1)* d.avg * co2_conv),
#         sdNEEuncorr = if (nRec == 0)
#           NA_real_
#         else sqrt(mean(NEE_uStar_fsd^2, na.rm = TRUE) / (nRec - 1)* d.avg * co2_conv)
#         ) %>% 
#   mutate( 
#     NEE_U = NEE + sdNEE, 
#     NEE_L = NEE - sdNEE)
...
#         nRec = sum(NEE_uStar_fqc == 0, na.rm = TRUE),
#         nEff = computeEffectiveNumObs(resid, effAcf = !!autoCorr, na.rm = TRUE),
#         NEE = mean(NEE_uStar_f, na.rm = TRUE)* d.avg * co2_conv * 365,
#         sdNEE = if (nEff <= 1)
#           NA_real_
#           else sqrt(mean(NEE_uStar_fsd^2, na.rm = TRUE) / (nEff - 1)* d.avg * co2_conv * 365),
#         sdNEEuncorr = if (nRec == 0)
#           NA_real_
#         else sqrt(mean(NEE_uStar_fsd^2, na.rm = TRUE) / (nRec - 1)* d.avg * co2_conv * 365)
#         ) %>% 
#   mutate( 
#     NEE_U = NEE + sdNEE, 
#     NEE_L = NEE - sdNEE)
...
#         )
# 
# sdAggDayUstar <- cbind(aggDayUstar$DateTime, transform(aggDayUstar[ , c(3:6)], 
#                            SD=apply(aggDayUstar,1, sd, na.rm = TRUE))) %>% 
#                   mutate(
#                     date = aggDayUstar$DateTime,
#                     NEE = ustar,
#                     NEE_U = ustar + SD,
#                     NEE_L = ustar - SD,
#                   )
...
#   ddply("period", summarize, 
#         DateTime = first(sDateTime), 
#         ustar = mean(NEE_uStar_f, na.rm = TRUE)* d.avg * co2_conv * 365.5,
#         U05 = mean(NEE_U05_f, na.rm = TRUE)* d.avg * co2_conv * 365.5,
#         U50 = mean(NEE_U50_f, na.rm = TRUE)* d.avg * co2_conv * 365.5,
#         U95 = mean(NEE_U95_f, na.rm = TRUE)* d.avg * co2_conv * 365.5,
#         )
...
# 
# 
# #--------- GS/NGS aggregation for random uncertainty ---------#
# 
# # number of obs per season
# count <- REddyProcOutput %>%
#   count(period, season)
...
#   date = aggDayRand$date,
#   sdRand = aggDayRand$sdNEE,
#   sdUstar = sdAggDayUstar$SD,
#   sdComb = sqrt(aggDayRand$sdNEE^2 + sdAggDayUstar$SD^2) 
# ) %>% 
#   mutate(
#     NEE = aggDayRand$NEE,
#     NEE_U = aggDayRand$NEE + sdComb,
