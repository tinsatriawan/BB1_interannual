...
```{r HOS approach for NEE using weekly data}
...
## transformation 
daily_nGF1 <- daily_nGF %>%
  mutate(co2gC = log(co2 + 1 - min(co2))^2,
         TS.5 = (TS.5 + 1 - min(TS.5))^3,
         LE = log(LE + 1 - min(LE))^(2),
         wtd = exp(wtd + 1 - min(wtd))) %>%
  dplyr::select(-c(Ta, VPD.y, H, PARin))  # remove due to multicollinearity
...
for(i in 1: length(period)){
  if(i == 1){ 
    co2_MLR_yearly <- calc_yearly_MLR(daily_nGF1, period[i], scope, "co2") %>% 
  mutate(period = period[i])
  } else { 
    co2_MLR_yearly <- rbind(co2_MLR_yearly, 
                            calc_yearly_MLR(daily_nGF1, period[i], scope, "co2") %>% 
                              mutate(period = period[i]) )}
