...
model_2 <- lm(co2gC ~ TS.5 + wtd + LE, data = weekly_nGF1) 
sum_2 <- summary (model_2)
sum_2 
...
daily_nGF1 <- daily_nGF %>%
  mutate(co2gC = log(co2gC + 1 - min(co2gC))^2,
         ...
         LE = log(LE + 1 - min(LE))^(2),
         wtd = exp(wtd + 1 - min(wtd))) %>%
  dplyr::select(-c(Ta, VPD.y, H, PARin))  # remove due to multicollinearity
...
scope <-  list(upper = ~TS.5 + RH + Precip + PA_2M + wtd + LE + ustar, 
                                   lower = ~1)
...
for(i in 1: length(period)){
  ...
  mutate(period = period[i])
    ...
    co2_MLR_yearly <- rbind(co2_MLR_yearly, 
                            calc_yearly_MLR(daily_nGF1, period[i], scope, "co2gC") %>% 
                              mutate(period = period[i]) )}
 
  }


co2_MLR_yearly$variable <- substr(co2_MLR_yearly$variable,2, 5)
...
#          co2gC = log10(co2gC + 1 - min(co2gC))^3,
#          TS.5 = ((TS.5 + 1 - min(TS.5))^5),
#          Ta = ((Ta + 1 - min(Ta))^3),
#          wtd = log10(wtd + 1 - min(wtd))^2,
#          LE = LE,
#          H = log10(H + 1 - min(H))^8) %>%
#   dplyr::select(-c(PARin, VPD.y))
...
model_x <- lm(ch4gC ~ wtd + LE + GPP + TS.5 , data = weekly_nGF1) 
...
model_2 <- lm(ch4gC ~ TS.5 + wtd + LE + GPP, data = weekly_nGF1) 
sum_2 <- summary (model_2)
sum_2 
...
model_2_sep <- lm(ch4gC ~ (GPP + wtd + LE + TS.5) * period, data = weekly_nGF1)
sum_2_sep <- summary(model_2_sep)
sum_2_sep
...
daily_nGF1 <- daily_nGF %>%drop_na()%>%
  mutate(ch4gC = (log10(ch4gC + 1 - min(ch4gC)))^0.6,
         # co2gC = (exp(co2gC + 1 - min(co2gC)))^2,
         GPP_f_RF = (GPP_f_RF + 1 - min(GPP_f_RF))^3, 
         TS.5 = exp((TS.5 + 1 - min(TS.5)))^2,
         Ta = (exp(Ta + 1 - min(Ta)))^2,
         wtd = (exp(wtd + 1 - min(wtd)))^0.8,
         LE = log10(LE + 1 - min(LE))^2,
         H = (H + 1 - min(H))) %>%
  dplyr::select(-c(PARin, VPD.y, co2gC, H, Ta)) # remove due to multicollinearity

cor(daily_nGF1%>% dplyr::select(-c(date, period)))


# define the variables to be included in the model
scope <-  list(upper = ~GPP_f_RF + TS.5 + RH + 
                       Precip + PA_2M + wtd + LE + ustar, 
               lower = ~1)
...
for(i in 1: length(period)){
  ...
  mutate(period = period[i])
    ...
    ch4_MLR_yearly <- rbind(ch4_MLR_yearly,
                            calc_yearly_MLR(daily_nGF1, period[i], scope, "ch4gC") %>%
                              mutate(period = period[i]) )}

  }


ch4_MLR_yearly$variable <- substr(ch4_MLR_yearly$variable,2, 5)
